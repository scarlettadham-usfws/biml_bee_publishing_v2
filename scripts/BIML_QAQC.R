...
##had to comment out old part to deal with delimiters - hilola_adham@fws.gov
##d <- readr::read_delim(file = "data/USGS_DRO_flat.txt.gz", delim = '$')
# Add quote = "" to disable quote parsing
# Note the quote = "" argument. This is required!
...
FLAGGED <- d %>%
  mutate(
    ...
    # ---  FIX THE DATE ERROR ---
    DeterminedWhen = lubridate::ymd(DeterminedWhen, quiet = TRUE),
    # -------------------------------------------
